...
class MG(object):
    ...
    def solve(self, b, x0=None, A=None, maxiter=40, tol=1e-8,
              cycle_type='V',
              accel={'module': ('pyamg', 'gmres'),
                     "resid": 'rel', 'restart': 20},
              ):
        """
        ...
        if solver_type == 'cycle':
            ...
            cb = CallBack()
            x1, info = scipy_gmres(A, b, x0=x0, tol=tol,
                                   maxiter=maxiter,
                                   M=M, callback=cb,
                                   restart=restart)
            ...
            out = pyamg_fgmres(A, b, x0=x0, tol=tol,
                               # inner iteration
                               restrt=restart,
                               # outer iter
                               maxiter=int(maxiter / restart),
    ...
    def get_solution_timings(self):
        ...
        frame = frame.loc[(frame != 0).any(axis=1)]  # Remove rows with all 0s in a Dataframe
        # sum up each column and sort decreasing order
        idx = np.argsort(np.nansum(frame.values, axis=0))[::-1]
        # reorder the frame
        sorted_frame = frame[frame.columns[idx]]
