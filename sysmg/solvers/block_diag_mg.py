...
class BlockDiagMG(MG):
    ...
    def _setup_solver(self):
        ...
        if self.pressure_solve['solver'][0] in ['diag', 'sa_amg']:
            ...
            raise ValueError("Only block-diag solver has been implemented for pressure.")
    ...
    def aspreconditioner(self, cycle='V'):
        ...
        def mv(r):
            ...
            x[:nv] = self.alpha*self.ml.solve(r[:nv], x0=None, tol=1e-50,
                                   maxiter=self.gamma, cycle=cycle)
            if self.type == 'uzawa':
                x[nv:] -= self.beta*(Mp @ (r[nv:] - B * x[:nv]))
            elif self.type == 'physics':
                x[nv:] = Mp @ r[nv:]
            else:
                raise ValueError("Block diagonal solver can be of two types physics or Uzawa.")
