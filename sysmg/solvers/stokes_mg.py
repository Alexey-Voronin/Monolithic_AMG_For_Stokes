...
class StokesMG(MG):
    ...
    def __init__(self, stokes_system, mg_param,
        ...
        if sys_order == "low":
            ...
            raise ValueError('Please provide system order for Stokes_AMG: options are "low" or "high".')
    ...
    def _setup_hierarchy(self):
        ...
        if self.interpolation['type'] == 'algebraic':
            ...
            raise ValueError('Interpolation type %s not defined.' \
                             % self.interpolation['type'])
    ...
    def _setup_relaxation(self, levels):
        ...
        if wrapper_params:
            ...
            self.wrapper = HighOrderWrapper(self.ho_system, wrapper_params, keep=self.keep)
        ...
        for i, level in enumerate(levels):  # exact solve on level[-1]
            ...
            if not self.keep:
                del level.A_bmat
                for mat in ['mass_bmat', 'stiffness_bmat', 'P_bmat', 'R_bmat', 'A_bmat']:
    ...
    def _get_amg_interp(self, **kwargs):
        ...
        if hasattr(p_params, 'drop_tol'):
            raise ValueError("""Parameter depricated. Pass in as tuple to
                            Agg/Smooth-Mat.""")
        ...
        for lvl in range(nlvls):
            ...
            Pv_lvl_bmat = [[None for _ in range(dim)] \
                           for _ in range(dim)]
            for i in range(dim):
                Pv_lvl_bmat[i][i] = Pv_lvl[i]
            Pv_lvl_bmat = sp.bmat(Pv_lvl_bmat, format='csr')
            # Add pressure interp
            Pi = BlockMatrix([[Pv_lvl_bmat, None],
                              [None, Pp_lvl[0]]])
            Ps_bmat.append(Pi)
    ...
    def __repr__(self):
        ...
        output += f'Operator Complexity: {ml.operator_complexity():6.3f}:' \
                  + O_ml + '\n'
        ...
        for n, lvl in enumerate(ml.levels):
            ...
            output += f'{n:>6} {A.shape[1]:>11} {A.nnz:>12} [{stk_ratio:>5}%]' \
                      + f'{vp_ratio:>8.1f}'
            # coarsening rate
    ...
    def to_dict(self):
        ...
        if self.dim == 2:
            add_fields = {'CR(u_x)': {}, 'CR(u_y)': {}, 'CR(p)': {},
                          'dofs(u_x)': {}, 'dofs(u_y)': {}, 'dofs(p)': {},
                          'O(ml)': {}, 'O(ml(A))': {},
                          'O(u_x)': {}, 'O(u_y)': {}, 'O(p)': {},
                          'eigs(MA)': {},
                          }
        else:
            add_fields = {'CR(u_x)': {}, 'CR(u_y)': {}, 'CR(u_z)': {}, 'CR(p)': {},
                          'dofs(u_x)': {}, 'dofs(u_y)': {}, 'dofs(u_z)': {}, 'dofs(p)': {},
                          'O(ml)': {}, 'O(ml(A))': {},
                          'O(u_x)': {}, 'O(u_y)': {}, 'O(u_z)': {}, 'O(p)': {},
                          'eigs(MA)': {}
                          }
        mg_info.update(add_fields)
        mg_info.update({'Aloc(mean)': {},
                        'Aloc(std)': {},
                        'Aloc(min)': {},
                        'Aloc(max)': {},
                        'Aloc(sum)': {},
        ...
        for lvl, level in enumerate(self.ml.levels):
            ...
            elif presmoother == None and lvl == len(self.ml.levels)-1 \
                    and hasattr(self, 'wrapper') and \
                    self.wrapper.outer_relaxation != None and\
                    hasattr(self.wrapper.outer_relaxation, 'patch_stats'):
