...
class Vanka(System_Relaxation):
    ...
    def __init__(
        ...
        assert self.pdof_order in [
            "natural",
            "sorted",
        ], 'patch_order must be "natural" or "sorted"'
        ...
        self._udofs_component_wise = getattr(
            stokes, "_udofs_component_wise", [self.M.shape[0] // self.dim] * self.dim
        )
        ...
        if self.debug:
            ...
            dofs = (
                np.vstack((vdofs, vdofs, pdofs))
                if dim == 2
                else np.vstack((vdofs, vdofs, vdofs, pdofs))
            )
            GS = self._GS

            dofs_remapped = GS.T * np.einsum(
                "ij,i->ij", GS * dofs, self._partition_of_unity
            )
            ...
            assert np.logical_or(
                hasZeroCoord, hasOneCoord
            ).all(), "GS operator does not work.."
        else:
            # not needed anymore
            delattr(self, "B")
            delattr(self, "M")
    ...
    def _compute_block_sizes(self):
        ...
        if self.vanka_type in ["algebraic_n_part", "algebraic_factorized"]:
            self.block_sizes = (self.B.indptr[1:] - self.B.indptr[:-1] + 1).astype(
                np.int64
            )
            ...
            raise NotImplementedError(
                f"Vanka type {self.vanka_type} _compute_block_sizes not implemented."
            )
    ...
    def _get_patch(self, p_idx):
        ...
        if not isinstance(p_idx, Iterable):
            ...
            v_ids = self.B.indices[
                self.B.indptr[p_idx[0]] : self.B.indptr[p_idx[-1] + 1]
            ]
        else:
            # get non-zero divergence values (Bdata)
            # and the associated column indices (v_ids)
            l_slice = slice(0, 0)
        ...
        if len(p_idx) == 1:
            ...
            _Aloc[: Mloc.shape[0], -1] = Block
            _Aloc[-1, : Mloc.shape[0]] = Block
        else:
            Block = Block.toarray()
            _Aloc = np.zeros((Mloc.shape[0] + 3, Mloc.shape[0] + 3))
            _Aloc[: Mloc.shape[0], : Mloc.shape[0]] = Mloc.toarray()
            _Aloc[: Mloc.shape[0], Mloc.shape[0] :] = Block.T
            _Aloc[Mloc.shape[0] :, : Mloc.shape[0]] = Block
    ...
    def _assemble_scatter(self, pressure_dofs_order=None):
        ...
        if self.vanka_type in ["algebraic", "algebraic_n_part"]:
            pdofs_order = (
                pressure_dofs_order
                if pressure_dofs_order is not None
        ...
        _GS = sp.csr_matrix((data, (rows, cols)), shape=(nrows, ncols), dtype=bool)
        self._GS = _GS
    ...
    def _assemble_partition_of_unity(self):
        ...
        if self._GS is not None:
            ...
            raise NotImplementedError(
                "First setup _GS operator before calling _assemble_partition_of_unity"
            )
    ...
    def _patch_setup_algebraic_n_part(self):
        ...
        if self.pdof_order == "natural":
            ...
            raise NotImplementedError(
                "patch_order = {} not implemented".format(self.patch_order)
            )
        ...
        if self.vanka_setup_opt:
            ...
            for p_idx in Ps_ordered:
                ...
                self._A_inv_flat[q_slice] = self._patch_solver(
                    _Aloc, wrap=False
                ).ravel()
        ...
        if self.vanka_opt:
            # needed for batching
            self.patch_size_and_counts = np.vstack(
                np.unique(self.block_sizes, return_counts=True)
            ).T
    ...
    def _patch_setup_algebraic_factorized(self):
        ...
        if self.vanka_setup_opt:
            ...
            M_inv_nnz_offset_no_padding = (
                0  # needed to keep track of where we are in M_inv
            )
            B_nnz_offset = 0
            ...
            for p in range(n_p):
                ...
                for Ux_size in Mloc_x_sizes[p]:
                    M_inv_i = M_inv[
                        M_inv_nnz_offset_no_padding : M_inv_nnz_offset_no_padding  # Patch vector Laplacian
                        + Ux_size**2
                    ]  # will go here
        ...
        for p in range(n_p):  # Traversal order is important!!!
            u_ids = B.indices[B.indptr[p] : B.indptr[p + 1]]
            nv = u_ids.shape[0]
            rows[offset : offset + nv] = np.linspace(
                offset, offset + nv - 1, nv, dtype=int
            )
    ...
    def _patch_setup_geometric_dg(self):
        ...
        if self.pdof_order == "natural":
            ...
            P_cells = self.system.dof_cells[-1]["p"]
            Ps_ordered = np.linspace(
                0, P_cells.shape[0] - 1, P_cells.shape[0], dtype=int
            )
        elif self.pdof_order == "sorted":
            Ps_ordered = np.argsort(self.block_sizes)
            self.block_sizes = self.block_sizes[Ps_ordered]
        ...
        if self.vanka_setup_opt:
            ...
            for pcell, bs in zip(P_cells, block_sizes):
                ...
                self._A_inv_flat[q_idx : q_idx + Aloc.size] = self._patch_solver(
                    Aloc, wrap=False
                ).reshape(-1)
                l_idx += bs
                q_idx += Aloc.size
    ...
    def _set_additive_relaxation_method(self):
        ...
        if self.vanka_opt and self.vanka_type in ["algebraic_n_part", "geometric_dg"]:
            ...
            # precompute indices
            self._l_slice = np.zeros(
                (self.patch_size_and_counts.shape[0] + 1), dtype=np.int64
            )
            self._q_slice = np.zeros(
                (self.patch_size_and_counts.shape[0] + 1), dtype=np.int64
            )
            ...
            assert (
                self._A_inv_flat.shape[0] == self._q_slice[-1]
            ), "A_inv_flat size does not match the number of patches: {} != {}".format(
                self._A_inv_flat.shape[0], self._q_slice[-1]
            )
            assert (
                self._r_gs.shape[0] == self._l_slice[-1]
            ), "r_gs size does not match the number of patches: {} != {}".format(
                self._r_gs.shape[0], self._l_slice[-1]
            )
            ...
            if self.cblas:
                ...
                def mv(A, x, b):
                    ...
                    for _ in range(self.relax_iters):
                        ...
                        n_part_batch(  # offset
                            ...
                            self._A_inv_flat,
                            self._x_gs,
                            # partition of unity and omega scaling
                            self._partition_of_unity,
                            self.omega,
                        )
                ...
                def mv(A, x, b):
                    ...
                    for _ in range(self.relax_iters):
                        ...
                        for i, (patch_size, count) in enumerate(
                            self.patch_size_and_counts
                        ):
                            ...
                            if count == 1:
                                ...
                                Ainv_batch = self._A_inv_flat[
                                    q_slice[i] : q_slice[i + 1]
                                ].reshape((patch_size, patch_size))
                                np.dot(Ainv_batch, r_batch, out=r_batch)
                            else:
                                r_batch = r_gs[l_slice[i] : l_slice[i + 1]].reshape(
                                    (-1, patch_size)
                                )
                                Ainv_batch = self._A_inv_flat[
                                    q_slice[i] : q_slice[i + 1]
                                ].reshape((count, patch_size, patch_size))
                                np.einsum(
                                    "pij,pj->pi",
                                    Ainv_batch,
                                    r_batch,
