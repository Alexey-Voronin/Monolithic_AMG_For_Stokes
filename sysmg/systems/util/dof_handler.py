...
class DoFHandler(object):
    ...
    def __init__(self):
        ...
        DoFHandler class is resposible for dealing with degree of
        freedom orderings.
        """
        pass
    ...
    def _construct_dof_coord(self):
        ...
        for Vm in [p.Z for p in self.problems]:
            coords = {}
            for V, elem in zip(Vm.subfunctions if type(elems) in [tuple, list] else Vm,
                               elems if type(elems) in [tuple, list] else [elems]):
                mesh = V.mesh()
                order = V.ufl_element().degree()
                Vspace = VectorFunctionSpace(mesh, elem, order)
                coord_field = interpolate(SpatialCoordinate(mesh),
                                          Vspace).dat.data_ro.copy()
    ...
    def _get_component_splitter(self, nDoFs):
        ...
        Pv = sp.csr_matrix((np.ones(len(rows)), (rows, cols)),
                           shape=(nv, nv), dtype=int)
    ...
    def _get_field_sorting_operator(self, coord):
        ...
        Nx = coord.T.shape[0]
        P = sp.coo_matrix((np.ones(len(rows)),
                           (np.array(rows), np.array(cols))),
                          shape=(Nx, Nx), dtype=int).tocsr()

        return P, dof_map

    @timeit("setup:system:DoFHandler:")
    def _get_sv_to_th_operator(self, V_dg, V_cg, Pp_dg_sort, nv):
        # Exctract trial/test functions from VectorFunction Spaces
        ...
        try:
            ...
            ml_solve = smoothed_aggregation_solver(M_cg, presmoother=smoother,
        ...
        # Put everything into a linear operator for seemless use
        ...
        def mv(up_dg):
            ...
            if ml_solve is not None:
                up_cg[nv:] = ml_solve.solve(T_dg_to_cg * up_dg[nv:],
                                            accel='fgmres',
                                            maxiter=100,
                                            tol=1e-12,
                                            residuals=resid)
    ...
    """ functions as an idenity operator
    def _get_cg_to_dg_operator2(self, V_dg, V_cg, Pp_dg_sort, nv):
        ...
        # Assemble Operators and Excract CSR matrices
        def _extract_op(A):
            tmp  = A.petscmat
            return sp.csr_matrix(( tmp.getValuesCSR())[::-1])
    ...
    def _get_cg_to_dg_operator(self, cg, dg):
        ...
        P_1to1 = sp.csr_matrix((np.ones(n_dg), (cols, rows)),
                               shape=(n_dg, n_cg))
    ...
    def _grid_hierarchy(self):
        ...
        self.grid_hier = []
        self.ext_grid_hier = []
        for dof_coord, Z, NEx, NEy in zip(self.dof_coord,
                                          [p.Z for p in self.problems],
                                          NEx_hier, NEy_hier):
            ...
            for name, dof_coord in dof_coord.items():
                ...
                order = order if isinstance(self.elem_order, tuple) \
                    else self.elem_order
                assert order in [1, 2], \
                    'Grids for element of order=%d are not supported yet.' \
                    % order
                ###############################################################
                ...
                ext_grid = self._extend_dof_grid(fine_grid, bc_type=bc_type,
                                                 padding=offset)
    ...
    def _extend_dof_grid(self, grid, bc_type=None, padding=0):
        ...
        if padding > 0:
            pad = padding
            if bc_type == 'periodic':
                ext_grid = np.ones((grid.shape[0] + pad * 2,
                                    grid.shape[1] + pad * 2),
                                   dtype=int) * -1
                ...
                # corners
                ext_grid[:pad, :pad] = grid[(ly - pad):(ly - 0),
                                       (lx - pad):(lx - 0)]
                ext_grid[(ly + pad):, (lx + pad):] = grid[:pad, :pad]
                ext_grid[:pad, (lx + pad):(lx + pad * 2)] = grid[(ly - pad):(ly - 0),
                                                            :pad]
                ext_grid[(lx + pad):(lx + pad * 2), :pad] = grid[:pad,
                                                            (ly - pad):(ly - 0)]
            else:
                ext_grid = np.ones((grid.shape[0] + pad * 2,
                                    grid.shape[1] + pad * 2), dtype=int) * -1
                ext_grid[pad:-1 * pad, pad:-1 * pad] = grid
