...
def bary_reg_ref_mesh(mesh):
    """
    Work for all meshes in serial. Parallel behaviour is ill-defined,
    however there are asserts that will fail in case there is an issue.
    """
    dm = bary(mesh.topology_dm)
    ...
    if dim == 3:
        dm_facets = dm.getDepthStratum(2)
        num_dm_facets = dm_facets[1] - dm_facets[0]

    dm_cells_arr = []
    # dm_coords_arr = numpy.zeros((num_dm_vertices,2))
    ...
    for dm_cell in range(*dm_cells):
        ...
        for p in dm_cell_vertices:
            dm_coords_arr[p - dm_vertices[0], :] = dm_coords(p)

    dm_cells_arr = numpy.array(dm_cells_arr)
    dm_coords_arr = numpy.array(dm_coords_arr)
    ...
    # Setup Boundary Markers for the bary mesh
    b_plex = dm  # bary mesh plex
    b_coords = b_plex.getCoordinates()
    b_coord_sec = b_plex.getCoordinateSection()
    b_plex.createLabel(dmcommon.FACE_SETS_LABEL)
    b_plex.markBoundaryFaces("boundary_faces")
    ...
    if plex.getStratumSize("boundary_faces", 1) > 0:
        ...
        for face, bface in zip(boundary_faces, b_boundary_faces):
            ...
            label_id = plex.getLabelValue(dmcommon.FACE_SETS_LABEL, face)

            plex.setLabelValue(dmcommon.FACE_SETS_LABEL, face, b_label_id)
...
""" old - to be deleted
def bary_reg_ref_mesh(unit_mesh):
    dm = bary(unit_mesh.topology_dm)
    ...
    from pyop2.mpi import COMM_WORLD
    comm = dm.comm
    plex = plex_from_cell_list(dim, dm_cells_arr, dm_coords_arr, COMM_WORLD)
    ...
    coord_sec = plex.getCoordinateSection()
    if plex.getStratumSize("boundary_faces", 1) > 0:
        xcoords = dm_coords_arr[:,0]
        ycoords = dm_coords_arr[:,1]
        boundary_faces = plex.getStratumIS("boundary_faces", 1).getIndices()
        xtol = 0.5*min(xcoords[numpy.where(xcoords > 0)[0]])
        ytol = 0.5*min(ycoords[numpy.where(ycoords > 0)[0]])
        x0   = min(xcoords); x1 = max(xcoords)
        y0   = min(ycoords); y1 = max(ycoords)
