...
def get_q1isoq2_basis(component="y"):
    ...
    phi_sq12 = sym.Piecewise(
        (sq1[1], (x <= Lx / 2) & (y <= Ly / 2)),
        (sq2[0], (x >= Lx / 2) & (y <= Ly / 2)),
        (0, True),
    )
    phi_sq24 = sym.Piecewise(
        (sq2[3], (y <= Ly / 2) & (x >= Lx / 2)),
        (sq4[1], (y >= Ly / 2) & (x >= Lx / 2)),
        (0, True),
    )
    phi_sq13 = sym.Piecewise(
        (sq1[2], (y <= Ly / 2) & (x <= Lx / 2)),
        (sq3[0], (y >= Ly / 2) & (x <= Lx / 2)),
        (0, True),
    )
    phi_sq34 = sym.Piecewise(
        (sq3[3], (x <= Lx / 2) & (y >= Ly / 2)),
        (sq4[2], (x >= Lx / 2) & (y >= Ly / 2)),
        (0, True),
    )
    phi_sqr1234 = sym.Piecewise(
        (sq1[3], (x <= Lx / 2) & (y <= Ly / 2)),
        (sq2[2], (Lx / 2 <= x) & (y <= Ly / 2)),
        (sq3[1], (x <= Lx / 2) & (y >= Ly / 2)),
        (sq4[0], (x >= Lx / 2) & (y >= Ly / 2)),
...
def element_mass_matrix(phi, symbolic=True, Omega_e=None, pretty=False, name=""):
    ...
    for r in range(n):
        for s in range(r, n):
            I = sym.integrate(
                sym.integrate(phi[r] * phi[s] * detJ, (X, 0, 1)), (Y, 0, 1)
            )
...
def my_mesh(nx, ny, x=[0, 1], y=[0, 1]):
    ...
    internal_v = list(
        set(
            np.linspace(0, len(v_dof_coord) - 1, len(v_dof_coord), dtype=int)
        ).difference(bcs_v)
    )
    ...
    internal_p = list(
        set(
            np.linspace(0, len(p_dof_coord) - 1, len(p_dof_coord), dtype=int)
        ).difference(bcs_p)
    )
    ...
    return (
        coords,
        cells,
        (p_dof_coord, p_dof_map, internal_p),
        (v_dof_coord, v_dof_map, internal_v),
    )


def element_divergence_matrix2(
    p_phi_in, v_phi_in, Omega_e, symbolic=True, component="x"
):
    n_p = len(p_phi_in)
    n_v = len(v_phi_in)
    ...
    # need to ontroduce coordinate system for differentiation
    p_phi = p_phi_in
    v_phi = v_phi_in  # []
    x, y = sym.symbols("x y")
    ...
    for r in range(n_p):
        for s in range(n_v):
            ...
            #                                (x, 0, 1)), (y, 0, 1))
            I = sym.integrate(p_phi[r] * v_phi[s], (x, 0, 1), (y, 0, 1))
...
def assemble_divergence2(
    vertices, elements, p_data, v_data, p_phi, v_phi, symbolic=False, component="x"
):
    ...
    for e in range(N_e):
        if Adiv_e is None:
            ...
            hx = max(Omega_e_x[1] - Omega_e_x[0], Omega_e_y[1] - Omega_e_x[0])
            Adiv_e = (
                element_divergence_matrix2(
                    p_phi, v_phi, Omega_e=Omega_e, symbolic=False, component=component
                )
                * hx**2
            )
