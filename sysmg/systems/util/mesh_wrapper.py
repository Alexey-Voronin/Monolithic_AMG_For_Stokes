...
class MeshWrapper(object):
    ...
    def __init__(self, params):
        ...
        if type(params) is str:
            ...
            self.quadrilateral = (
                self.mesh.topology.ufl_cell().cellname() == "quadrilateral"
            )
            self.boundary_marker = {"left": 0, "right": 1, "bottom": 2, "top": 2}

        elif (
            isinstance(params, MeshGeometry)
            ...
            self.dim = self.mesh.topological.ufl_cell().geometric_dimension()
            ...
            if self.dim == 1:
                ...
                if n_markers == 4:
                    ...
                    if np.min(self.markers) == 1:
                        self.boundary_marker = {"inflow": 1, "sides": 3, "outflow": 2}
                    else:
                        self.boundary_marker = {"inflow": 0, "sides": 2, "outflow": 1}
                else:
                ...
                if n_markers == 6:
                    ...
                    self.boundary_marker = {"inflow": 1, "sides": 0, "outflow": 2}
                else:
                    raise ValueError("new mesh markers?")
            else:
            ...
            assert isinstance(
                params, dict
            ), """MeshWrapper input is not valid.
            ...
            if not self.mesh_type:
                if dim == 1:
                    ...
                    if n_markers == 6:
                        ...
                        raise ValueError(
                            """You are probably passing in a new
            ...
            assert len(self.markers) == len(
                set(self.boundary_marker.values())
            ), """Number of mesh markers (%d) does not match the
                    boundary marker (%d).%s""" % (
                len(self.markers),
                len(self.boundary_marker.values()),
                str(self.boundary_marker),
            )
    ...
    def _get_refinement_info(self):
        """Get number of refinement levels from.
        ...
        if self.mesh_hierarchy:
            if not np.alltrue(np.array([ne % 2 for ne in NE]) == 0):
                raise ValueError(
                    """MeshWrapper->_get_refinement_info: Need even #
    ...
    def _setup_2D_bfs_meshes(self):
        ...
        if not self.quadrilateral:
            raise Exception(
                """_setup_2D_bfs_meshes: Triangular Mesh looks
