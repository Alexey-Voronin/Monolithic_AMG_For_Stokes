...
class StokesProblem(Problem):
    def __init__(
        ...
        if elem_type[0] == "CG" and elem_type[1] == "CG":
            ...
            raise ValueError(
                "Discretization with element types {str(elem_type) not recognized."
            )
    ...
    def function_space(self, msh, elem_type, order, name=("u", "p"), scalar=False):
        ...
        if elem_type[1] == "DG":
            ...
            element = FiniteElement(
                elem_type[1], msh.ufl_cell(), degree=order[1], variant="equispaced"
            )
            P = FunctionSpace(msh, element, name=name[1])
        else:
            P = super().function_space(
                msh, elem_type[1], order[1], name[1], scalar=True
            )
        self.Z = U * P
        return self.Z
    ...
    def bcs(
        ...
        if bc_type in [None, "periodic"]:
            ...
            center = float((mesh_max[0] + mesh_min[0]) / 2.0)
            ...
            center = float((mesh_max[0] + mesh_min[0]) / 2.0)
            if abs(center) < 1e-6:
                # here to accommodate airfoil mesh, which is defined
                # over domain [-8,8]x[-8,8]
                ms = 1
                fx = -((ms * abs(x)) ** 2) + (mesh_max[0] * ms) ** 2
            else:
                # should be able to accommodate mesged defined over
                # domains [0,L]x[0,L]
            ...
            bm = boundary_markers
            lid_id = (bm[lid_side],)
            other_ids = tuple([s for s in bm.values() if s != lid_id[0]])
    ...
    def plot_solution(
        self, up, items=["mesh", "u", "div u", "p", "grad p"], save_name=None
    ):
...
if __name__ == "__main__":
    ...
    problem.assemble_and_plot(
        "lid-driven-cavity", {"top": 4, "bottom": 3, "left": 1, "right": 2}
    )
    # Scott-Vogelius (2,1) on structured mesh w/ in-out-flow bcs
    problem = StokesProblem(mesh, elem_order=(2, 1), elem_type=("CG", "DG"))
    problem.assemble_and_plot(
        "in-out-flow", {"top": 4, "bottom": 3, "left": 1, "right": 2}
    )
