...
class System(MeshWrapper, DoFHandler):
    ...
    def plot_guess(self, u, mesh=False):
        ...
        To be overwritten by implemnting class.
        """
        pass
    ...
    def _homogeneous_bcs(self, bcs_type, component='u'):
        ...
        if bcs_type is None:
            ...
            for (i, mesh), Vm in zip(enumerate(meshes), self.space_hier):
                ...
                val = tuple([0] * dim) \
                    if isinstance(Velem, VectorElement) else 0.0
    ...
    def get_mass_matrix(self, test, trial, bcs, block=(0, 0)):
        ...
        Mass = assemble(integrand * dx, bcs=bcs,
                        mat_type='nest', sub_mat_type='aij')
        if block == None:
            return sp.csr_matrix((Mass.petscmat.getValuesCSR())[::-1])
        else:
            return sp.csr_matrix((Mass.petscmat.getNestSubMatrix(*block) \
                                  .getValuesCSR())[::-1])
    ...
    def get_stiffness_matrix(self, test, trial, bcs, block=(0, 0), elem_type='CG'):
        ...
        if block == None:
            ...
            return sp.csr_matrix((Stiffness.petscmat.getNestSubMatrix(*block) \
                                  .getValuesCSR())[::-1])
