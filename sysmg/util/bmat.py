...
class BlockMatrix(object):
    ...
    def update_shape(self):
        ...
        try:
            ...
            self.shape = (np.sum(np.array(Arow), axis=0).max().astype(int),
    ...
    def __sub__(self, other):
        ...
        bmat_new = BlockMatrix([[None for _ in range(self._ncols)] for _ in range(self._nrows)])
        for i in range(self._nrows):
            for j in range(self._ncols):
    ...
    def __mul__(self, other):
        ...
        bmat_new = [[None for _ in range(self._ncols)]\
            ...
            for j in range(other._ncols):
                ...
                for k in range(self._ncols):
                    ...
                    if lmat != None and rmat != None:
                        assert lmat.shape[1] == rmat.shape[0], \
                            'Dimension mismatch A_{%d,%d}xB_{%d,%d}.' % (i,k,k,j)

                        if sum_ is None:
                            sum_ = sp.csr_matrix((lmat.shape[0], rmat.shape[1]),
                                                  dtype=np.float64)
                        sum_ +=  lmat*rmat
    ...
    def __getattr__(self, attr):
        if attr == 'T':
            AT = BlockMatrix([[None for _ in range(self._nrows)]
...
if __name__ == '__main__':
    ...
    for _ in range(10):
        ...
        assert np.allclose(true_result, bmat_result), \
                'BlockMatrix multiplication is broken'
        # subtraction test
        assert np.linalg.norm((R-R).tocsr().data) < 1e-8, \
                'BlockMatrix subtraction is broken'
        # addition test
        K2 = (bmat12+bmat12).toarray()
        assert np.allclose(true_result*2, K2), \
                'BlockMatrix addition is broken'
        # Transpose test
        my_K_diff = (K - K.T).toarray()
        assert np.allclose(my_K_diff, true_K_diff), \
                'BlockMatrix transpose is broken'
