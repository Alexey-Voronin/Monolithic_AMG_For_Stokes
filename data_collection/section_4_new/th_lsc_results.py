...
from palettes import get_section_4_th_attr
...
########################################################################
########################################################################
names, colors, markers, linestyles = get_section_4_th_attr(rlx="LSC-DGS")
ms = 3
########################################################################
# LOAD DATA
########################################################################
...
rlx_type = "lsc_dc"
p2p1_dir = f"amg_p2p1/{rlx_type}/"
isop2p1_dir = f"amg_isop2p1/{rlx_type}/"
...
for (prob_path, data_dict), ax in zip(all_problems.items(), axs):
    ...
    # y-ticks
    # ax.set_yticks([0.25, 0.5, 0.75, 1.0, 1.25, 1.5, 1.75, 2.00 ])
    ax.set_yticks([0.5, 1.0, 1.5, 2.00])
    ax.yaxis.set_minor_locator(MultipleLocator(0.125))
...
if "--savefig" in sys.argv:
    plt.savefig("p2p1_lsc_results.pdf")
else:
    plt.show()
