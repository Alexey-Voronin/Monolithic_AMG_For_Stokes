...
data_paths = {names[0] : {'2D Structured'   : PREAM + "eta_ho/structured/2D_bfs/",
              ...
              names[1] : {'2D Structured'   : PREAM + "eta_hlo/structured/2D_bfs/",
...
def print_opt_values(rho, iters, ETAs):
    """ Optimal paramters based on the smallest number of iterations is more
...
cf_min   = [1e4]*ncols; cf_max   = [-1]*ncols
iter_min = [1e4]*ncols; iter_max = [-1]*ncols
for pid, mg in enumerate(data_paths.keys()):

    datas = data_paths[mg]
    for i, (prob_type, PATH) in enumerate(datas.items()):
        axes = AXES[i]
        ...
        axes.plot(indep_var, iters,
                  label=mg,
                  linestyle='-', lw=1,
                  clip_on=False,
                  color=colors[pid],
                  marker=markers[pid],
                  markersize=markersize, markevery=5)
        ...
        axes.plot(eta_opt_reported, iters_opt,
                  marker=markers[pid],
...
AXES[1].legend(handles=[handles[0]], labels=[labels[0]],
           bbox_to_anchor=(0., 1.2, 1.0, 0.2),
           loc='upper center', ncol=1
           )
AXES[2].legend(handles=[handles[1]], labels=[labels[1]],
           bbox_to_anchor=(0., 1.2, 1.0, 0.2),
           loc='upper center', ncol=1
           )
...
for j, ax in enumerate(AXES):
    ...
    ax.set_xticks(np.linspace(indep_var[0]*0.975, indep_var[-1]*1.01, len(xticks)), xticks)
