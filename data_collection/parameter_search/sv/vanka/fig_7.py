...
def print_opt_values(rho, iters, ETAs, OMEGAs):
    ...
    print(f'rho-based:  (omega_0, eta_p)=({omega_opt:2.3f}, {eta_opt:2.3f}):\titer={iters_min:1.3f}\trcf={rho_min:1.3f}')
    #################################################################
    # based on iter
    ...
    print(f'iter-based: (omega_0, eta_p)=({omega_opt:2.3f}, {eta_opt:2.3f}):\titer={iters_opt:1.3f}\trcf={rho_min:1.3f}')
...
########################################################################
PREAM      = "eta_omega/"
data_paths = {'Structured'   : PREAM+"structured/2D/",
...
for ax, (msh_type, PATH) in zip(AXES, data_paths.items()):
    ...
    im = ax.contourf(X, Y, iters,
                     cmap='hot',
                     levels=np.arange(21, 40)+0.5,
                     vmin=22, vmax=39,
                     origin='lower',
                     );
...
cb = fig.colorbar(im, ticks=np.arange(22, 41))
