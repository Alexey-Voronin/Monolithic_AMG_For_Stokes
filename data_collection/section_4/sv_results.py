...
from palettes import get_section_4_sv_attr
...
# paths[ ] below.
########################################################################
names, colors, markers, linestyles = get_section_4_sv_attr()
ms = 3
...
PREAM_LO_1 = COMM_PREAM + "defect_correction/lsc_dc/"
...
for i, ((prob_path, data_dict), ax) in enumerate(zip(all_problems.items(), axs)):
    for j, (k, v) in enumerate(data_dict["residuals"].items()):
        ...
        ax.semilogx(
            ...
            marker=markers[j],
            markersize=ms,
            mfc=("none" if "LSC" in k else colors[j]),
            linewidth=linewidth,
...
axs_all[0, 0].legend(  # custom_lines, names,
    loc="lower left",
    bbox_to_anchor=(0.1, 1.2, 3.9, 0.2),
    mode="expand",
    borderaxespad=0,
    ncol=3,
)
