...
# paths[ ] below.
########################################################################
names   = get_robustness_plot_names("p2p1disc")
...
PREAM_LO    = COMM_PREAM + "defect_correction/"

all_problems = dict.fromkeys(["structured/2D_bfs/",
                              "unstructured/2D/",
                             ], None)

for problem in all_problems.keys():

    paths     = [PREAM_HO + problem,
                 PREAM_LO + problem,
                ]
...
fig, axs_all = plt.subplots(nrows, ncols, #sharey='row',
                            sharex='col')

# Ylabels (titles of plots - one per row)
for i, title in enumerate(["iterations",
                           "relative time",
                           "rel. time per iter."]):
...
ymin = 1e4; ymax = -1
for i, ((prob_path, data_dict), ax) in enumerate(zip(all_problems.items(),
                                                     axs)):
    ...
    for j, (k,v) in enumerate(data_dict['residuals'].items()):
        ...
        ax.semilogx(size, iters, label=k,
                    linestyle='-',
                    color=colors[j],
                    marker=markers[j],
                    markersize=ms,
                    linewidth=linewidth, clip_on=False, zorder=10)
...
for i, ((prob_path, data_dict), ax) in enumerate(zip(all_problems.items(), axs)):
    ...
    for j, (k,v) in enumerate(data_dict['timings'].items()):
        ...
        # since the relative timings need to be commputed and some AMG types
        ...
        ax.semilogx(dofs, rel_time, label=k,
                    linestyle='-',
                    color=colors[j],
                    marker=markers[j],
                    markersize=ms,
...
for (prob_path, data_dict), ax in zip(all_problems.items(), axs):
    ...
    for j, ((k,v), resid_hist) in enumerate(zip(data_dict['timings'].items(),
                                 data_dict['residuals'].values())):
...
for r, axs in enumerate(axs_all):
    ...
    for (i, data_dict), data in zip(enumerate(all_problems.values()),
                                              all_problems.values()):

        ndofs = np.array([int(i) for i in
                          data_dict['timings'][names[0]].keys()])
...
axs_all[0,1].legend(#custom_lines, names,
                    loc='lower left',
                    bbox_to_anchor=(-0.1, 1.2, 1.2, 0.2),
